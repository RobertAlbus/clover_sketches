...
struct patch_automation_t {
    patch_automation_t(float fs, float bpm, bar_grid_029& grid);

    // --------------------------------
    // SIDECHAIN

    ducker_props sc_pump_props;
    env_bp_040_props hp_env_kick_hp;
};
...
struct patch {
    patch(float fs, float bpm, bar_grid_029& grid) : automation{fs, bpm, grid} {
    }
