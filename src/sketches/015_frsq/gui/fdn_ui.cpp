...
void fdn_component(const std::string& id, fdn_8_props_012& fdn_props_L, fdn_8_props_012& fdn_props_R) {
    ...
    if (SliderFloat("fb", &fdn_props_L.fb_gain.gui, 0, 2)) {
        fdn_props_L.fb_gain.store(fdn_props_L.fb_gain.gui);
        fdn_props_R.fb_gain.store(fdn_props_L.fb_gain.gui);
    }
    ...
    if (SliderFloat(
        ...
        fdn_props_L.lpf_cut.store(fdn_props_L.lpf_cut.gui);
        fdn_props_R.lpf_cut.store(fdn_props_L.lpf_cut.gui);
    }
    ...
    if (SliderFloat(
                ...
                "%.3f",
                ImGuiSliderFlags_Logarithmic | ImGuiSliderFlags_NoRoundToFormat)) {
        fdn_props_L.hpf_cut.store(fdn_props_L.hpf_cut.gui);
        fdn_props_R.hpf_cut.store(fdn_props_L.hpf_cut.gui);
    }
    ...
    Text("%.2f", fdn_props_L.fb_gain.gui);
    ImGui::TableNextColumn();
    Text("%.2f", fdn_props_L.fb_gain.load_output());
    ...
    Text("%.2f", fdn_props_L.lpf_cut.gui);
    ImGui::TableNextColumn();
    Text("%.2f", fdn_props_L.lpf_cut.load_output());
    ...
    Text("%.2f", fdn_props_L.hpf_cut.gui);
    ImGui::TableNextColumn();
    Text("%.2f", fdn_props_L.hpf_cut.load_output());
    ImGui::EndTable();
    ...
    for (auto [i, tap_L, tap_R] :
        ...
        if (SliderFloat("fdl_tap", &tap_L.gui, 4, 12000)) {
            tap_L.store(tap_L.gui);
            tap_R.store(tap_L.gui);
        }
